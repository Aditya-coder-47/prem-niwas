...
export async function getBuildingSettings(): Promise<BuildingSettings> {
  try {
    const docRef = doc(db, 'building_settings', 'prem_niwas');
...
export async function uploadQRPaymentImage(file: File | Blob): Promise<string> {
  const storagePath = `payment_qr/prem_niwas_qr.jpg`;
  try {
    const storageRef = ref(storage, storagePath);
    await uploadBytes(storageRef, file);
...
export async function recordPayment(
  ...
  const receiptId = `PREM-${params.roomNumber}-${dateCode}-${Math.floor(100 + Math.random() * 900)}`;
